...
router.post("/submit", verifyTokenMiddleware, async (req, res) => {
  const { weekdaysPeople, weekendsPeople, applianceCounts } = req.body.more;
  ...
  try {
    ...
    // CSV content
    const csvContent =
      `Monitored Weekdays People,Weekends People,Continues,Standby,Cold,Active\n` +
      `${weekdaysPeople},${weekendsPeople},${applianceCounts.continuous},${applianceCounts.standby},${applianceCounts.cold},${applianceCounts.active}`;
    ...
    return res
      .status(500)
      .json({ error: "An error occurred while submitting data" });
  }
});
